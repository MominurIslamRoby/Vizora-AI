...
import { Loader2, BookOpen, Atom, ScrollText, Dna, Microscope, Globe, Compass } from 'lucide-react';
...
const Loading: React.FC<LoadingProps> = ({ status, step, facts = [] }) => {
  ...
  const FlyingItem = ({ delay, position, type, content }: { delay: number, position: number, type: 'icon' | 'text', content: any }) => {
    ...
    return (
      <div 
        className={`absolute flex items-center justify-center font-bold opacity-0 select-none ${type === 'text' ? 'text-cyan-600 dark:text-cyan-300 text-[10px] md:text-xs tracking-[0.2em] bg-white/80 dark:bg-slate-900/90 border border-cyan-500/30 px-2 py-0.5 md:px-3 md:py-1 rounded shadow-[0_0_10px_rgba(6,182,212,0.3)] backdrop-blur-sm' : 'text-amber-500 dark:text-amber-300'}`}
        style={{
  ...
  return (
    <div className="relative flex flex-col items-center justify-center w-full max-w-4xl mx-auto mt-8 min-h-[350px] md:min-h-[500px] overflow-hidden rounded-3xl bg-white/40 dark:bg-slate-900/40 border border-slate-200 dark:border-white/10 shadow-2xl backdrop-blur-md transition-colors">
      ...
      {/* THE REACTOR CORE - Using the New Logo */}
      <div className="relative z-20 mb-10 md:mb-16 scale-[0.65] md:scale-125 mt-4 md:mt-10">
        ...
        <div className="relative p-1 animate-[pulse-core_2.5s_infinite]">
      ...
      <div className="relative z-30 w-full max-w-lg bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl rounded-2xl p-6 md:p-8 shadow-2xl border border-slate-200 dark:border-white/10 text-center flex flex-col items-center transition-all duration-500 min-h-[140px] md:min-h-[160px]">
